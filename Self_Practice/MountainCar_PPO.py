...
def run_agent(time=1000):
    # model = DQN.load(_MountainCar')
    model = PPO.load('PPO_MountainCar/best_model')
    obs = env.reset()
...
def csv_results():
    data = np.load('PPO_MountainCar/evaluations.npz')
    ...
    df.to_csv('PPO_MountainCar.csv')

def plot():
    df = pd.read_csv('PPO_MountainCar.csv')
    time = df['Time Steps'].to_numpy()
    mean = df['Average Total Reward'].to_numpy()
    plt.plot(time, mean)
    plt.xlabel('Time Steps (millions of steps)')
...
if __name__ == '__main__':
    ...
    eval_callback = EvalCallback(eval_env, best_model_save_path="./PPO_MountainCar/",
                                log_path="./PPO_MountainCar/", eval_freq=1000,
