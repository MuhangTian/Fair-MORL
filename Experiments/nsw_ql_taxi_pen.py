...
def run_NSW_Q_learning(episodes, alpha, epsilon, gamma, nsw_lambda, init_val, dim_factor, tolerance, alpha_N, file_name, mode):
    ...
    np.save(file='taxi_q_tables_V1/NSW_Penalty_V2_size{}_locs{}_{}{}'.format(fair_env.size,len(fair_env.loc_coords), aN, file_name),
            arr=Q)
    np.save(file='taxi_q_tables_V1/NSW_Penalty_V2_size{}_locs{}_{}{}_loss'.format(fair_env.size,len(fair_env.loc_coords), aN, file_name),
            arr=loss_data)
...
if __name__ == '__main__':
    ...
    prs.add_argument("-tj", dest="episodes", type=int, default=40000, required=False, help="episodes.\n")
